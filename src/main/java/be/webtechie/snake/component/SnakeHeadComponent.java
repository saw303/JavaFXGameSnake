...
public class SnakeHeadComponent extends Component {
  ...
  public void onUpdate(double tpf) {
    ...
    if (apple.getPosition().equals(previousPosition)) {
      ...
      apple.translate(
          FXGLMath.random(0, getAppWidth() - 20), FXGLMath.random(0, getAppHeight() - 20));
    }
  }

  private void checkForBounds() {
    if (entity.getX() < 0) die();

    if (entity.getX() >= getAppWidth()) die();

    if (entity.getY() < 0) die();

    if (entity.getY() >= getAppHeight()) die();
  }
  ...
  public void die() {
    ...
    bodyParts.stream().skip(1).forEach(Entity::removeFromWorld);
